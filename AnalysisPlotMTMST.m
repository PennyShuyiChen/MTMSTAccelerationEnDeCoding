...
for u = 1:nunits
    ATI(u)=(max(dv.tune.trMu(:,u))-min(dv.tune.trMu(:,u)))./...
        (max(dv.tune.trMu(:,u))- mean(dv.tune.trMu(:,u)));

    VTI(u)=(max(dv.tune.tuneMuspd(u,:))-min(dv.tune.tuneMuspd(u,:)))./...
        (max(dv.tune.tuneMuspd(u,:))-mean(dv.tune.tuneMuspd(u,:)));
end
...
if exist('dvT')
    ...
    for u = 1:nunits
        if ~isnan(prefDir(u))
            DTI(u)=(collapseDirtr(prefDir(u),u)-collapseDirtr(nullDir(u),u))./...
                (collapseDirtr(prefDir(u),u)-mean(collapseDirtr(:,u)));

            BDTI(u)=(max(collapseDisptr(:,u))-min(collapseDisptr(:,u)))./...
                (max(collapseDisptr(:,u))-mean(collapseDisptr(:,u)));
        else
            DTI(u) = nan;
            BDTI(u) = nan;
        end
